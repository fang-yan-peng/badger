  ...
  private static class InvokerInfo {
    ...
    public InvokerInfo(Class<?> clazz) throws Exception {
      ...
      for (PropertyMeta pm : BeanUtil.fetchPropertyMetas(clazz)) {
        ...
        FunctionalGetterInvoker fgi = FunctionalGetterInvoker.create(name, pm.getType(),readMethod);
        gim.put(name, fgi);
        gis.add(fgi);
        FunctionalSetterInvoker fsi = FunctionalSetterInvoker.create(name,pm.getType(), writeMethod);
        sim.put(name, fsi);
        sis.add(fsi);
      }
